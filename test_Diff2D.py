...
sim = DiffFluidSim2D(num_particles=10, max_timesteps=max_steps, do_print_stats=False)
sim.initialize()

target = np.array([30, 15])
...
steps = max_steps-1
sim.loss[None] = 0
sim.emit_particles(1, 0, np.array([2,2]), initial_vel)
...
print("Pos final: ", sim.positions[2,0][0], sim.positions[2,0][1])
