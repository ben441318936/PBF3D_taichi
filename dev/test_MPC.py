...
actual_sim = HandGradSim3D(max_timesteps=301, num_particles=3600, do_save_npy=True, do_emit=True)
aux_sim = HandGradSim3D(max_timesteps=10, num_particles=3600, do_save_npy=False, do_emit=True)
...
for i in range(aux_sim.max_timesteps):
    init_tool_states[i,:] = np.array([0.1, 0.0, 3])
...
for i in range(1,300):
    actual_sim.take_action(i, np.array([3.0, 3.0, 3.0]))
